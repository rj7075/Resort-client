...
const RoomDetails = () => {
  ...
  const checkAvailability = async () => {
    try {
      // check is checkin date grater that checkout date
      if (checkInDate >= checkOutDate) {
        toast.error("CheckIn Date should be less than checkout Date");
        return;
      }

      const { data } = await post("/api/bookings/check-availability", {
        room: id,
        checkInDate,
        checkOutDate,
      });
  ...
  // onsubmit handler function tocheck availability and book the room
      ...
      if (!isAvailable) {
        ...
        const { data } = await post(
          "/api/bookings/book",
          {
            ...
            checkInDate,
            checkOutDate,
            guests: Number(guest), // ✅ plural, numeric
    ...
    room && (
      <div className="md:py-35 py-28 px-4 md:px-16 lg:px-24 xl:px-32">
        ...
        >
          ...
          >
            {isAvailable ? "Book Now" : "Check Availavility"}
          </button>
        </form>
        ...
        <div className="flex flex-col items-start gap-4">
          <div className="flex gap-4">
            <img
              src={room.hotel.owner?.avatar || assets.userPlaceholder}
              alt="Host"
